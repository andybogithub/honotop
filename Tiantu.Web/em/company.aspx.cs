...
public partial class em_company : System.Web.UI.Page
{
                ...
                foreach (var item in dsList)
                {
                    ...
                    <a href = 'companyde.aspx?id={0}' >
                        <div class='clist_cont'>
                            ...
                            <div class='dowmload'><a href='{4}' download><input type = 'button' value='Download'></a></div>	
                        </div>
                    </a>
                </div>", item.NOTICEID, "", item.TITLE, item.PUBDATE.ToString("yyyy-MM-dd"),item.PDFURL_EN==""?item.PDFURL:item.PDFURL_EN);
                }
            }
        }
        return dataString;
