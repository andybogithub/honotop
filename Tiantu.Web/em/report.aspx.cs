...
public partial class em_report : System.Web.UI.Page
{
            ...
            if (dsList != null)
            {
                foreach (var item in dsList)
                {
                    dataString += string.Format(@" <li>
                            ...
                            <div  class='download'>
                                <a href='{2}' download>Download</a>
                            </div>
                            <p>{3}</p>
                        </li>", item.REPORTID, item.IMGURL, item.PDFURL_EN=="" ? item.PDFURL : item.PDFURL_EN, item.TITLE);
                }
            }
